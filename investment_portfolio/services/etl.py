...
@transaction.atomic
def load_weights_and_prices(file: UploadedFile):
    """
    Load Excel data from uploaded file to database
    """
    ...
    try:
        # Read Sheets
        weights_df = pd.read_excel(tmp_path, sheet_name='weights')
        prices_df = pd.read_excel(tmp_path, sheet_name='Precios')
